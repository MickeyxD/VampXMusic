...
from VampXMusic.config import ARQ_API_KEY
from VampXMusic.config import BOT_NAME as bn
from VampXMusic.config import DURATION_LIMIT
from VampXMusic.config import UPDATES_CHANNEL as updateschannel
from VampXMusic.config import que
from VampXMusic.function.admins import admins as a
from VampXMusic.helpers.admins import get_administrators
from VampXMusic.helpers.channelmusic import get_chat_id
from VampXMusic.helpers.errors import DurationLimitError
from VampXMusic.helpers.decorators import errors
from VampXMusic.helpers.decorators import authorized_users_only
from VampXMusic.helpers.filters import command, other_filters
from VampXMusic.helpers.gets import get_file_name
from VampXMusic.services.callsmusic import callsmusic, queues
from VampXMusic.services.callsmusic.callsmusic import client as USER
from VampXMusic.services.converter.converter import convert
from VampXMusic.services.downloaders import youtube
        ...
        for administrator in administrators:
            if administrator == message_.from_user.id:
                ...
                try:
                    ...
                    await lel.edit(
                        f"<b>🔴 Flood Wait Error 🔴 \nUser {user.first_name} couldn't join your group due to heavy requests for userbot! Make sure user is not banned in group."
                        "\n\nOr manually add assistant to your Group and try again request here @VampBot_Support with your group link</b>",
                    )
    try:
        await USER.get_chat(chid)
        # lmoa = await client.get_chat_member(chid,wew)
        ...
        await lel.edit(
            f"<i> {user.first_name} Userbot not in this chat, Ask admin to send /play command for first time or add {user.first_name} manually request here @VampBot_Support with your group link</i>"
        )
        return
        ...
        for administrator in administrators:
            if administrator == message_.from_user.id:
                ...
                try:
                    ...
                    await lel.edit(
                        f"<b>🔴 Flood Wait Error 🔴 \nUser {user.first_name} couldn't join your group due to heavy requests for userbot! Make sure user is not banned in group."
                        "\n\nOr manually add @VCPlayBot to your Group and try again request here @VampBot_Support with your group link</b>",
                    )
    try:
        await USER.get_chat(chid)
        # lmoa = await client.get_chat_member(chid,wew)
        ...
        await lel.edit(
            "<i> helper Userbot not in this chat, Ask admin to send /play command for first time or add assistant manually request here @VampBot_Support with your group link</i>"
        )
        return
    requested_by = message_.from_user.first_name
    chat_id = message_.chat.id
